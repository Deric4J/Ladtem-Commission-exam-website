...
const AdminDashboard: React.FC = () => {
  const { exams, submissions, activityLogs, allUsers } = useApp();
  ...
  const stats = [
    ...
    { label: 'Avg Pass Rate', value: '84%', color: 'text-amber-600', icon: <Icons.Search /> }
  ];
             ...
             <div className="space-y-4">
               {submissions.filter(s => s.status === 'COMPLETED').slice(0, 3).map(sub => (
                 <div key={sub.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                   <div>
                     <p className="text-sm font-bold text-slate-800">{allUsers.find(u => u.id === sub.studentId)?.name}</p>
                     <p className="text-[10px] text-slate-400 uppercase font-bold">Scored {sub.score} Pts</p>
                   </div>
                   <Badge variant={sub.score && sub.score > 5 ? 'success' : 'warning'}>
                     {sub.score && sub.score > 5 ? 'Excellent' : 'Needs Review'}
                   </Badge>
                 </div>
               ))}
               {submissions.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No grading data yet.</p>}
             </div>
          </Card>
          ...
          {activityLogs.slice(0, 10).map(log => (
            <div key={log.id} className="flex gap-3 items-start pb-4 border-b border-slate-50 last:border-0">
               <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs ${
                 ...
                 log.type === ActivityType.LOGIN ? 'bg-blue-50 text-blue-600' : 'bg-indigo-50 text-indigo-600'
               }`}>
                 {log.type === ActivityType.LOGIN ? <Icons.Users /> : <Icons.Plus />}
               </div>
               <div>
        ...
        {studentsOnly.map(student => {
          ...
          return (
            <Card key={student.id} className="hover:border-blue-200 transition-all group">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      {student.institute} • {student.matricNumber} • {student.email}
                    </p>
                  </div>
                </div>
              ...
              {/* Individual Records Expanded */}
              {selectedStudent?.id === student.id && (
                <div className="mt-8 pt-8 border-t border-slate-100 animate-in slide-in-from-top-4">
                  <h5 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Detailed Performance Logs</h5>
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    {submissions.filter(s => s.studentId === student.id).length === 0 && (
                      <p className="col-span-2 text-center text-sm text-slate-400 italic py-4">No completed exams found for this candidate.</p>
                    )}
                  </div>
                  <Button variant="outline" className="mt-6 w-full" onClick={() => setSelectedStudent(null)}>Close Records</Button>
                </div>
              )}
            </Card>
          );
